...
public:
    ...
    MatF SolveMultiShape(std::vector<ProjectionParameters> params, std::vector<MatF> landMarks, std::vector<MatF> modelMarks,std::vector<float>& angles, MatF SB, float lambda)
    {
        //cout << Shape(SB) << endl;
        ...
        {
            totalLandMarkNum+=landMarks[i].rows();
            //std::cout<<"landMarks["<<i<<"]:"<<landMarks[i].rows()<<","<<landMarks[i].cols()<<std::endl;
        }

        int rowIndex=0;
        int L = SB.cols();
        MatF SBX(totalLandMarkNum * 2, L);
        MatF error(totalLandMarkNum * 2,1);
        for(size_t i=0;i<imageNum;i++)
        {
            float weighti=std::abs(std::cos(angles[i]));
            MatF Ri = params[i].R * params[i].s;
            Ri = Ri.block(0, 0, 3, 2);
            ...
            {
                SBXi.block(ii * 2, 0, 2, L) = Rit * SB.block(ii * 3, 0, 3, L);
            }
            if (USEWEIGHT)
            {
                Matrix<float, Eigen::Dynamic, 1> W = Matrix<float, Eigen::Dynamic, 1>::Ones(2 * Ni, 1);
                for (size_t j = 0; j < SkipList.size(); j++)
                {
                    W(2 * SkipList[j] + 0, 0) = WEIGHT;
                    W(2 * SkipList[j] + 1, 0) = WEIGHT;
                }
                SBXi = W.asDiagonal() * SBXi;
                errori = W.asDiagonal() * errori;
            }
            SBXi*=weighti/params[i].s;
            errori*=weighti/params[i].s;
            error.block(rowIndex,0,Ni*2,1)=errori;
            SBX.block(rowIndex,0,Ni*2,L)=SBXi;
            rowIndex+=Ni*2;
        }
